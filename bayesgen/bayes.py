...
class BayesGraph:
    ...
    def add_node(self, node):
        ...
        assert node.name not in self.node_name_index
...
probability ( {n.name} | {", ".join(n.condition_node_names)} ) """ + "{"
  ...
  ({", ".join(str(x) for x in r[0])}) {", ".join(str(x) for x in r[1])};"""
    ...
    def to_json(self):
        # Write to json file based on graph representation generated by https://github.com/vangj/spark-bbn
        nodes = []
        for n in self.nodes:
            cpts = []
            # condition -> prs
            condition_to_prs = dict((tuple(c), prs) for c, prs in n.rows)
            
            # the following steps are necessary to ensure that conditions are ordered correctly when generating cpts
            conds = []
            for cond in n.condition_nodes:
                conds.append(cond.output_vals)
            combos = itertools.product(*conds)
            for combo in combos:
                cpts += condition_to_prs[combo]
            
            node = {
                'id': n.name,
                'name': n.name,
                'values': n.output_vals,
                'cpts': cpts
            }
            nodes.append(node)
